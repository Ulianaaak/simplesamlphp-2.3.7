...
$config = [
    ...
    'Education' => array(
        'idp' => 'SAMLIDP',
        'sqlauth:SQL',
        'orcale_connection' => "oci:dbname=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=".$db_config['host_ip'].")(PORT=".$db_config['port']."))(CONNECT_DATA=(SERVICE_NAME=".$db_config['service_name'].")));charset=UTF8",
        'dsn' => 'oci:dbname=' . $tns . ';charset=UTF8',
        'charset' => 'utf8',
        'username' => 'do_user',
        'password' => 'warrior12345',
        'authQuery' => "SELECT 
						u.USERNAME as CURRENT_USER_NAME,
						u.USERNAME as CURRENT_USER_NAME_PROF, 
						'Education' as project,
						u.DELETED,
						u.PWD
						FROM AD_USERS  u WHERE u.USERNAME = :username ",
        'query' => "SELECT DISTINCT 
				u.USERNAME as CURRENT_USER_NAME,
				u.USERNAME as CURRENT_USER_NAME_PROF,
				'Education' as project,
				NVL2 (NULLIF( u.ISTEACHER, 0 ),NVL(u.PERSONID,u.STUDENTID) ,u.STUDENTID) as PERSONID,
				u.DELETED as deleted,
				u.ISTEACHER as isteacher,
				u.ISSTUDENT as student,
				u.ISINNER as isinner,
				u.ISEMPLOYER as isemployer,
				u.ISPARENT as isparent,
				u.ISLISTENER AS ISLISTENER,
               	CASE 
                	WHEN u.ISSTUDENT = 1 THEN sg.name||'('||d2.city||')'
                	WHEN u.ISLISTENER = 1 THEN sg.name||'('||d2.city||')'
                	WHEN u.ISTEACHER = 1 THEN 'teacher'
                	WHEN u.ISINNER = 1 THEN 'isinner'
                	WHEN u.ISEMPLOYER = 1 THEN 'isemployer'
                	WHEN u.ISPARENT = 1 THEN 'isparent'
                ELSE 'nogroup'
                END AS GROUPCASE,  
				NVL(p.FAMILYNAME,REGEXP_SUBSTR (u.USERFULLNAME, '[^ ]+',1,1)) as lastname,
                NVL(p.FIRSTNAME, REGEXP_SUBSTR (u.USERFULLNAME, '[^ ]+',1,2) ) as firstname,
                NVL(p.SECONDNAME,REGEXP_SUBSTR (u.USERFULLNAME, '[^ ]+',1,3) ) as middlename,
				nvl( ci.EMAIL, sr.CORPORATE_EMAIL) as email,
                NVL2 (
                    NULLIF( u.ISSTUDENT, 0 ),sg.name||'('||d2.city||')',
                    NVL2(
                    NULLIF( u.ISLISTENER, 0 ),sg.name||'('||d2.city||')',
                    NVL2(
    		        NULLIF( u.ISTEACHER, 0 ), 'teacher',
    		        NVL2(
    		        NULLIF( u.ISINNER, 0 ), 'isinner',
    		        NVL2(
    		        NULLIF( u.ISEMPLOYER, 0 ), 'isemployer',
    		        NVL2(
    		        NULLIF( u.ISPARENT, 0 ), 'isparent',
    		        'nogroup')))))) as GROUPNAME,
    		    NVL2 (
                    NULLIF ( u.ISSTUDENT, 0 ),'isstudent',
                    NVL2(
                    NULLIF( u.ISLISTENER, 0 ), 'islistener',
                    NVL2(
                    NULLIF( u.ISTEACHER, 0 ), 'isteacher',
                    NVL2(
                    NULLIF( u.ISINNER, 0 ), 'isinner',
                    NVL2(
                    NULLIF( u.ISEMPLOYER, 0 ), 'isemployer',
    		    ...
    		    REPLACE (
                    	'_'||
	    		        NVL2( NULLIF ( u.ISSTUDENT, 0 ),',isstudent','')||
	    		        NVL2(
	                    NULLIF( u.ISLISTENER, 0 ), ',islistener','')||
	                    NVL2(
	                    NULLIF( u.ISTEACHER, 0 ), ',isteacher','')||
	                    ...
	                    NULLIF( u.ISEMPLOYER, 0 ), ',isemployer','')||
	                    NVL2(
	                    NULLIF( u.ISPARENT, 0 ), ',isparent',''), '_,')
                     as idisrole1 ,
              	sg.code as idisgroupid,
                d2.city as city,
                decode (s.fgos,'3','3.5','3++','3.75') fgos,
                ep.planid idis_eduplan_id,
                ep.studyform,
				NVL2 (s.okso,
						CONCAT(
				                REPLACE(s1.OKSO,' ')
				                    ||'%'||trim(SUBSTR( SUBSTR(s1.name, 1,
				                        case when
				                            INSTR(s1.name,', профиль')=0
				                        then length(s1.name)
				                        else INSTR(s1.name,', профиль')-1 end
				                        ),1, case when
				                            INSTR(s1.name,'профиль')=0
				                        then length(s1.name)
				                        else INSTR(s1.name,'профиль')-1 end ))||'%'||
				                NVL2(s1.okso,'',REPLACE(s.okso,' '))
				                    ||'%',trim(s.name)||'%'),
				                NULL)  as eduplan,
				CASE s.SPECTYPE
				    WHEN 1 THEN 'Специалитет'
 				    WHEN 2 THEN 'Аспирантура'
				    WHEN 3 THEN 'Бакалавриат'
				    WHEN 4 THEN 'Магистратура'
				    WHEN 5 THEN 'подгот.фак(РКИ)' 
				    WHEN 10 THEN 'УПО'
				END as SPECTYPE,
				s.SPECTYPE as SKILLTYPE,
				ep.YEARBEGIN as YEARBEGIN,
				ep.SKILLCODE
                ,sig.ENDDATE
				FROM AD_USERS  u
                left JOIn FC_STUDENTRECORDS sr on (sr.recordid=u.STUDENTID)
                LEFT JOIN PS_PERSONS  p on (p.PERSONID=u.PERSONID or p.PERSONID=sr.PERSONID)
				LEFT JOIN CAB_CONTACT_INFO ci on ci.PERSONID=u.PERSONID
                LEFT JOIN FC_STUDENTSINGROUPS sig on (sig.STUDENTID=u.STUDENTID  and (sig.ENDDATE>:today or sig.ENDDATE is null) AND sig.BEGINDATE <= SYSDATE )
                LEFT JOIN FC_STUDENTGROUPS sg on ( sg.code=sig.groupid OR sg.code = u.groupid)
                left join RB_DEPARTMENTS d on d.code=sg.faculty
				left join PL_educplans ep on ep.planid=sg.planid
				left join RB_SPECIALITY s on ep.speccode=s.code
				left join RB_SPECIALITY s1 on s.PARENTSPEC=s1.code
                left join RB_DEPARTMENTS d2 on d2.code=d.PARENTDEPT
				WHERE u.USERNAME = :username  ",  // and  u.PWD=:password",//  or u.USERCODE=:password  ",
        'driver' => 'oci8',
        'failcount' => '3',
        'authbd' => 'idis',
    ),
